...
--c) Quantos projetos nao tem usuarios associados a ele. A coluna deve chamar qty_projects_no_users
...
--c) ID do projeto, nome do projeto, qty_users_project (quantidade de usuarios por projeto)
--em ordem alfabetica crescente pelo nome do projeto

select p.id, p.nome, COUNT(up.users_id) as qty_users_project
from projects p
inner join users_has_projects up
on p.id = up.projects_id
