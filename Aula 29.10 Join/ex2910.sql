...
select
	...
	al.nome, nt.nota as nota, av.peso as peso, nt.nota * av.peso as nota_calculada, mat.nome as disciplina
from alunos al
inner join alunomateria am
on al.ra = am.ra_aluno
inner join materias mat
on am.id_materia = mat.id
inner join notas nt
...
select
	substring(al.ra, 1, 9)+'-'+substring(al.ra, 10, 1) as ra,
	al.nome, mat.nome, nt.nota, av.tipo
from alunos al
inner join alunomateria am
on al.ra = am.ra_aluno
inner join materias mat
...
inner join avaliacoes av
on nt.id_avaliacao = av.id
where (av.tipo = 'P1' OR av.tipo = 'P2') AND nt.nota < 6.00
